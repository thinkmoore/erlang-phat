...
A single number cannot describe the failure threshold of a \phatraid{} cluster
because of the interplay between VR and RAID. The failure of the \phatraid{}
group is dependent on the arrangement of the failed nodes. For example, Figure
\ref{fig:failures_noprogress} depicts a situation wherein only four failures
stops progress. In contrast, Figure \ref{fig:failures_allok} depicts a situation
wherein five failures does \emph{not} stop progress.
...
When five nodes fail, they can be distributed into each group in the
proportions: (3,1,1), (3,2,0) and (2,2,1). The first case does not stop progress
...
\begin{align*}
  ...
  \textrm{5-Failure Arrs.}\;\;\;
    &\binom{9}{5} = 126 \\
  \textrm{Failure Prob.}\;\;\;
    &\Pr\left[\textrm{group failure} \,|\, n_{failed} = 5\right] = \frac{81 + 18}{126} =
    \frac{99}{126} \approx 0.79 \\
\end{align*}
...
to $2(f+1)$ failures and unlikely resistant to $2(f+1)+1$ failures.

The general form for the probability of \phatraidcf{$C$}{$f$} group failure
given $2(f + 1)$ node failures is:
