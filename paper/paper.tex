...
throughput, and the need to store large files with tolerance to some CPU,
network, and disk failure. In this setting, we reason that the major bottleneck
for replicated file storage is disk read/write latency. We combat this
bottleneck with ideas from RAID, which allow for faster disk access, and ideas
...
XOR delay versus 200 MB/second drive delay).
...
committed messages we haven't yet received.\footnote{This can happen if the
...
\ref{fig:failures}. In this section, ``data is lost'' means that, if no
persistent, fail-safe store is used, some data will be lost. Adding a
persistent, fail-safe store obviously prevents loss of committed
data. Additionally, we assume node failures are independent and uniformly
distributed throughout the \phatraid{} group.
...
progress; however, no data can been lost\footnote{Here and later in this paper
  ...
  cluster can be lost. In VR, by contrast, if data is decided and propagated
  throughout the cluster, that data can only be lost if every node is lost. We
  also assume the nodes do not have persistent, fail-safe stores.}. Progress is
blocked because more than one cluster cannot reach consensus. Since no cluster
...
because the group certainly cannot make progress if data has been lost.
...
because $C-1$ clusters can make progress, i.e. at least $f+1$ nodes are up in
...
because $C-1$ clusters have at least one node up and one cluster has no nodes
up.
...
When five nodes fail, they can be distributed into each group in three different
