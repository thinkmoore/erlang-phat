...
In a review of hard disk performance \cite{disk-perf}, Hard Disk Drives (HDD) were compared with Solid State Drives (SDD). HDDs use a physical magnetic platter that spins so that a drive head can read or write data by sensing or manipulating the magnetic field in a particular position on to the platter. In contrast, Solid SDD have no moving parts and instead uses integrated circuits as memory to store data persistently. Because the way in which data is accessed is different, the performance characteristics between the two are different.
...
Write Access Time & 0.03 - 0.32 seconds \\ \hline
Sequential Read Speed &  207.95 - 522.45 MB/sec \\
Read Access Time & 0.03 - 0.22 seconds
\end{tabular}
\caption[Solid State Performance]{Solid State Disk performance for middle 95\% of sold state drives tested in a benchmark. Benchmark available at Benchmark available at \hwbenchmark.}
\label{fig:sdd-perf}
\end{figure}

Figure \ref{fig:sdd-perf} shows the performance for SSDs from a 2013 survey of commercially available SDDs.
...
\begin{figure}
  \begin{tikzpicture}
    [->,scale=.8,]
    \node [rectangle, draw, thick] (raidclient) at (0,2) {Raid Client};
    \node [rectangle, draw, thick] (client1) at (-4,1) {Client 1};
    \node [rectangle, draw, thick] (client2) at (0,0) {Client 2};
    \node [rectangle, draw, thick] (client3) at (4,1) {Client 3};
    \node [rectangle, draw, thick] (vr11) at (-7, -2) {Master};
    \node [rectangle, draw, thick] (vr12) at (-5, -2) {Replica};
    \node [rectangle, draw, thick] (vr13) at (-3, -2) {Replica};
    \node [rectangle, draw, thick] (vr21) at (-2, -4) {Master};
    \node [rectangle, draw, thick] (vr22) at (0, -4) {Replica};
    \node [rectangle, draw, thick] (vr23) at (2, -4) {Replica};
    \node [rectangle, draw, thick] (vr31) at (3, -2) {Master};
    \node [rectangle, draw, thick] (vr32) at (5, -2) {Replica};
    \node [rectangle, draw, thick] (vr33) at (7, -2) {Replica};


    \foreach \to in {client1, client2, client3}
    \draw [thick] (raidclient) -- (\to);
    \foreach \to in {vr11, vr12, vr13}
    \draw [thick] (client1) -- (\to);
    \foreach \to in {vr21, vr22, vr23}
    \draw [thick] (client2) -- (\to);
    \foreach \to in {vr31, vr32, vr33}
    \draw [thick] (client3) -- (\to);
...
\subsection{Implenting RAID on \phat{}}
